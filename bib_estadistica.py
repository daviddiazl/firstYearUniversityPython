...
#rpr una alternativa mas eficiente, simlificando la fórmula 
def varianza(p): 
    """sec(numerico)-->float
    OBJ: varianza de la población p
    PRE: len(p)>0. media() definida"""
...
def coef_var(p):
    ...
    #como no recibe la media, no puedo poner PRE Media>0
    #de otra parte, aunque se pueda calcular, coef_var sin sentido si no p[i]>0, para todo i
    m=media(p)
    try:
        cv=desv_tipica(p)/m
    except:
    ...
    return  error,cv
'''
#o bien, si ya hubieramos estudiado el if
def coef_var(p):
    """sec(numerico)-->error, float
    OBJ: coeficiente de variación de la población p, error si media=0
    PRE: len(p)>0,. desv_tipica() y media() definidas"""
...
print('caso de prueba',(0,0,0),    'coeficiente variación= ',coef_var((0,0,0)))
'''

def variacion(m,k):
    """int,int-->int
    OBJ: variaciones de m elementos tomados de k en k
    PRE factorial() definida.  0<=k<=m """
